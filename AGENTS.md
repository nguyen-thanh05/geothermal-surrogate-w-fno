...
### Environment
Codex/local Python work for this repository should use the existing conda
environment named `torch`.

On Windows/PowerShell, prefer one of these forms:
```powershell
conda activate torch
python -c "import torch; print(torch.__version__)"
```

For non-interactive Codex shell commands, use:
```powershell
conda run -n torch python <script-or-module>
```

The expected interpreter is:
```text
C:\Users\ntt\anaconda3\envs\torch\python.exe
```
