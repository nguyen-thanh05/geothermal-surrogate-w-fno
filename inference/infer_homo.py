...
PRES_MIN, PRES_MAX = 1900, 68000.0 #4600.0, 33000.0
...
# Aux (BHP + energy rate) — ranges from ARFNO_onestep.py ARDataset
AUX_BHP_MIN, AUX_BHP_MAX = 1900.0, 68000.0
...
def load_datasets(data_path):
    """Load all dataset arrays (memory-mapped)."""
...
def load_model(cfg, checkpoint_path, device, use_ema=False):
    """Load LOGLO_FNO model (v1 or v2, per model.type) and AuxHead from config and checkpoint."""
...
def autoregressive_rollout(model, aux_head_model, gt_all, datasets, idx, device):
    ...
    with torch.no_grad():
        for t in range(N_STEPS):
            ...
            x_t = torch.cat([x_t, active_wells], dim=1)     # (1,2,D,H,W)

            predicted_y = model(y_t, x_t)
...
def plot_aux_pred_gt(pred_aux_all, datasets, idx, out_dir, traj_idx):
    ...
    pred_aux_all: (156,16) normalized predictions — BHP[7:16], energy[0:7].
    """
...
def make_action_pred_gt_gif(pred_all, gt_all, well_rates, sl, out_dir, traj_idx):
    ...
    """
    # We use a GridSpec: row 0 spans 9 cols, rows 1-3 span 4 cols each
    # To handle 9 action plots on top and 4 field columns below,
    # use a 36-column grid (LCM of 9 and 4).
    fig = plt.figure(figsize=(28, 22))
    gs = fig.add_gridspec(4, 36, hspace=0.35, wspace=0.4)

    # Row 0: 9 action subplots (each 4 cols wide)
    ax_actions = []
    for w in range(9):
        ax = fig.add_subplot(gs[0, w * 4: w * 4 + 4])
        ax_actions.append(ax)

    # Rows 1-3: 4 field columns (each 9 cols wide)
    ...
    # Set up action axes (fixed y-limits)
    action_lines = []
    action_vlines = []
    for w in range(9):
        ax = ax_actions[w]
    ...
    # Compute global colorbars for pred/GT and error
    ...
    # Error range (signed, unnormalized to physical units)
    ...
    # Create image objects for rows 1-3
    ...
    def update(frame_t):
        # Update action lines (show data up to frame_t)
        for w in range(9):
...
def eval_all(model, aux_head_model, datasets, device, sl, out_dir):
    ...
    # Accumulate signed error in physical units for mean
    ...
    for i, idx in enumerate(TEST_INDICES):
        ...
        # Signed error in physical units, mean over spatial dims
        for ch in range(4):
    ...
    # ── Plot A: Mean relative L2 across test set ──
    mean_l2 = all_l2.mean(axis=0)   # (4, 156)
    ...
    # ── Plot B: Mean signed (unnormalized) error across test set ──
    mean_signed = all_signed_err_mean.mean(axis=0)  # (4, 156)
...
# ── Main ─────────────────────────────────────────────────────────────────────
def main():
    parser = argparse.ArgumentParser(description='AR-FNO Inference')
    parser.add_argument('--config', type=str, default='configs/loglo_homo.yml',
