...
including UNet and Fourier Neural Operator variants, on 1200 anisotropic
three-dimensional fault-dominated geothermal simulations. Their models predict
3D temperature fields across eight discrete time steps,
demonstrating that neural surrogates can capture important spatiotemporal
patterns in geothermal systems.
...
A Neural Operator is defined as a function $\mathcal{G}: \mathcal{A} \ to \mathcal{U}$ where both $\mathcal{A}$ and $\mathcal{U}$ 
are Banach spaces defined on bounded domain. Fourier Neural Operator (FNO), following \cite{li2021fno}, consists of three 
components: a lifting operator, a sequence of Fourier layers, and a projection operator. Let $\mathcal{P}$ denote the lifting map, which embeds the input into a higher-dimensional latent space; let $\mathcal{L}_l$ denote the $l$-th Fourier layer; and let $\mathcal{Q}$ denote the projection map, which returns the latent representation to the target output dimension. The overall operator can then be written as
\begin{equation}
u = \mathcal{G}(a) = (\mathcal{Q} \circ \mathcal{L}_n \circ \cdots \circ \mathcal{L}_2 \circ \mathcal{L}_1 \circ \mathcal{P})(a).
\end{equation}
Each Fourier layer applies a spectral convolution by transforming the input to Fourier space using the Fast Fourier Transform, multiplying a selected set of low-frequency modes by learnable weights, and transforming the result back to physical space. This spectral update is combined with a pointwise neural network and passed through a nonlinear activation:
\begin{equation}
\mathcal{L}_l(x) = \sigma\left(\mathcal{F}^{-1}\left(\mathcal{F}(x) \odot \mathcal{W}_l\right) + \mathrm{MLP}(x)\right).
\end{equation}
Here, $\mathcal{W}_l$ denotes the learnable spectral weights for the $l$-th Fourier layer. 

FNO and other its variants achieved impressive performance on a wide variety of tasks, including [PLACEHOLDER].  
