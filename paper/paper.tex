...
time, enabling engineering workflows such as history matching, well placement,
...
networks (PINNs). PINNs learn solutions to partial differential
equations by modeling physical residuals, boundary conditions, and initial 
conditions as part of the training objective through automatic
differentiation. For example,
\cite{zhaoSurrogateModelingGeothermal2025} develop a PINN model to predict
...
energy in a PINN-based framework to predict two-phase transient gas-liquid flow
variables, including pressure, temperature, gas volume fraction, gas density,
liquid density, and gas velocity.

Although PINNs are attractive when the governing equations are available in a
tractable form, geothermal reservoir simulators often include physical and
geological complexity that is difficult to explicitly represent as a neural
...
inhibit the ability to write a
complete differentiable residual. In these settings, data-driven surrogate models
provide a practical alternative: instead of enforcing the governing equations
directly, these models learn the input-output behavior of the simulator from generated
trajectory data. Recent work has demonstrated cautiously optimistic successes 
of this data driven approach. 
\cite{liangSurrogateModelingGeothermal} train several deep learning models,
including variants of the UNet and Fourier Neural Operator (FNO), on 1200 anisotropic
...
In this work, we address these questions by proposing the following contributions:
\todo{rewrite contributions so it's more obvious what we did}
\begin{itemize}
  \item We release two 3D dual-porosity geothermal datasets (homogeneous and
        heterogeneous porosity/permeability) for benchmarking surrogate models;
  \item We adapt the Local--Global Fourier Neural Operator (LOGLO-FNO) to 3D
        non-isothermal geothermal simulation with several architectural and
        training modifications;
  \item We demonstrate a mean of less than\SI{2}{\percent} relative error and stable 156-step
